...
class ManageStoreView(View):
    def get(self, request):
        ...
        return render(request, "admin/manage_store.html", {"form": form, "store_list": store_list, 'store_category': store_category})
...
class ManageUpdateStoreView(View):
    def post(self, request, pk):
        ...
        if form.is_valid():
            ...
            store_category_id = request.POST.get('store_category')
            if store_category_id:
                category = get_object_or_404(StoreCategoryModel, id=store_category_id)
                updated_store.store_category = category
            # Handle new uploads
            new_image_urls = []
