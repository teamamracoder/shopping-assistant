...
#         user = UserModel.objects.get(id=40)
#         return user
...
class ManageUserProfileView(View):
    ...
    def get_user_instance(self, request):
        """
        Return currently logged-in user instance dynamically.
        Prefer request.user if authenticated; fallback to session-based get_user_id.
        """
    ...
    def post(self, request):
        user = self.get_user_instance(request)
        form = ManageUserProfileForm(request.POST, request.FILES, instance=user)
